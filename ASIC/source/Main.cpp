...
map<string, int> id;
map<set<string>,Truthtable* > cut_to_truth[10005];
...
void ins_trie(int s_trie[],int match){
    int x=0,i=0;
    while(i<(1<<k_trie)){
		int j=s_trie[i];
           ...
           // printf("1122\n");
                    ...
                    for (set<string> c1 : c.graph[input1]->cuts[t]) {
                        for (int k = 1; k <= Luts; k++)
                            for (set<string> c2 : c.graph[input2]->cuts[k]) {
                                ...
                                if(check_equivalence(ff))c.graph[now]->cuts[st.size()].insert(st),cut_to_truth[id[now]].insert(pair<set<string>, Truthtable *>(st, ff));
                                else
                                    delete ff;
                            }
                    }
                ...
                for (int k = 1; k <= Luts; k++)
                    for (set<string> c1 : c.graph[input1]->cuts[k]){
                            ...
                            if (check_equivalence(ff))
                                c.graph[now]->cuts[k].insert(c1), cut_to_truth[id[now]].insert(pair<set<string>, Truthtable *>(c1, ff));
                            else delete ff;
                    }
            }
            ...
            for (int k = 1; k <= Luts; k++)
            {
                for (set<string> ct : c.graph[now]->cuts[k]) {
                    ...
                    for(string xxs:ct)
                 //   printf("%s ", xxs.c_str() );
                  //  printf("\n import %d\n", check_equivalence(cut_to_truth[id[now]][ct]));
                    area = area_map[check_equivalence(cut_to_truth[id[now]][ct])];
                  //  printf("area: %lf %d %s\n", area,check_equivalence(cut_to_truth[id[now]][ct]),now.c_str());
                 //   for(string xxs:ct){
                //       printf("%s ",xxs.c_str());
               //    }
               //     printf("\n");
                    for (string nt : ct)
                    {
                        ...
                        area += c.graph[nt]->Area;
                        AF += c.graph[nt]->AreaFlow;
                  //      printf("### %s %s %d %d\n", now.c_str(), nt.c_str(),dep,c.graph[nt]->Area);
                    }
                    dep++;
            ...
            //printf("222###xxx%s %lf %lf %d\n", now.c_str(), Ct[0].Area,Ct[0].AreaFlow,Ct[0].mindep);
            //if(now=="new_n94_")return 0;
            //  jsqqqq++;
            //  if(jsqqqq==10)
            //     return 0;
        }
                ...
                if (c.graph[now]->pre.size() == 1) {
                    ...
                    for (int k = 1; k <= Luts; k++)
                        for (set<string> c1 : c.graph[input1]->cuts[k]){
                            Truthtable *ff = new Truthtable();
                            ff->get1(cut_to_truth[id[input1]][c1],c.graph[now]->plex);
                            if(check_equivalence(ff))c.graph[now]->cuts[k].insert(c1),cut_to_truth[id[now]].insert(pair<set<string>, Truthtable *>(c1, ff));
                            else delete ff;
                        }
                }
                //printf("yyzzy %s \n", now.c_str());
